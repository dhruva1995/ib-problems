package ib.hashing;

import java.util.HashMap;
import java.util.LinkedHashMap;

public class LongestSubstringWithoutRepeat {

	public static void main(String[] args) {
		System.out.println(new LongestSubstringWithoutRepeat().lengthOfLongestSubstring(
				""));
	}

	public int lengthOfLongestSubstring(String A) {

		int champLength = 0;

		HashMap<Character, Integer> lastVisited = new HashMap<>();
		int lastUpdated = -1;
		for (int i = 0; i < A.length(); i++) {
			char c = A.charAt(i);
			
			champLength = Math.max(champLength, lastVisited.size());
			
			if (lastVisited.containsKey(c)) {
				
				if (champLength <= i - lastUpdated - 1) {
					final int lastRepeatedIndex = lastVisited.get(c);
					lastVisited.entrySet().removeIf(e -> e.getValue() <= lastRepeatedIndex);
				}

			}
			lastVisited.put(c, i);
		}

		return Math.max(champLength, lastVisited.size());
	}

}
